...
public class FileWatcherService {
    ...
    private final NeosisConfigService neosisConfigService;
    private final ModeService modeService;

    @Value("${sgms.watch.debounce-create-ms:100}")
    private long debounceCreateMs;

    @Value("${sgms.watch.debounce-modify-ms:200}")
    ...
    private final ScheduledExecutorService scheduler = Executors.newSingleThreadScheduledExecutor(
        r -> {
            Thread t = new Thread(r, "neosis-watcher-debouncer");
            t.setDaemon(true);
            return t;
        }
    );
    ...
    private boolean isExcludedDirectory(Path dir) {
        ...
        if (pathStr.equals(".git") || pathStr.startsWith(".git/") ||
            ...
            pathStr.equals(".neosis") || pathStr.startsWith(".neosis/")) {
            return true;
        }
    ...
    private void watchDirectory() {
        while (!Thread.currentThread().isInterrupted()) {
            ...
            for (WatchEvent<?> event : key.pollEvents()) {
                ...
                if (neosisConfigService.shouldIndex(child)) {
