...
@Slf4j
@Component
@ConditionalOnProperty(name = "sgms.llm.provider", havingValue = "ollama", matchIfMissing = true)
@RequiredArgsConstructor
public class OllamaClient implements LlmClient {

    @Value("${sgms.llm.base-url:http://localhost:11434}")
    private String baseUrl;

    @Value("${sgms.llm.model:llama3.2:1b}")
    ...
    public List<AssertionExtractionResponse> extractAssertions(String sectionPath, String chunkContent) {
        ...
        try {
            ...
            if (response.statusCode() == 200) {
                ...
                String llmResponseText = root.path("response").asText();
                return parseJsonResponse(llmResponseText);
            } else {
                log.error("Ollama API failed with status {}: {}", response.statusCode(), response.body());
                throw new RuntimeException("LLM extraction failed");
            }

        } catch (Exception e) {
            log.error("Error during assertion extraction from LLM", e);
    ...
    private List<AssertionExtractionResponse> parseJsonResponse(String jsonString) {
        try {
            ...
            log.warn("Could not find JSON array in LLM response: {}", jsonString);
            return Collections.emptyList();
        } catch (JsonProcessingException e) {
            log.error("Failed to parse LLM JSON response: {}", jsonString, e);
            throw new RuntimeException("Failed to parse LLM JSON response", e);
        }
    }
}
