...
public class BulkProgressStore {
    ...
    private void loadFromRedis() {
        try {
            filesDiscovered.set(getInt("neosis:bulk:discovered", 0));
            filesProcessed.set(getInt("neosis:bulk:processed", 0));
            filesFailed.set(getInt("neosis:bulk:failed", 0));
            assertionsGenerated.set(getInt("neosis:bulk:assertions", 0));
            edgesGenerated.set(getInt("neosis:bulk:edges", 0));
            queueBacklog.set(getInt("neosis:bulk:backlog", 0));
            throughputDocsSec = getDouble("neosis:bulk:throughput", 0.0);
            etaSeconds = getInt("neosis:bulk:eta", 0);
    ...
    private void persistAll() {
        try {
            redisTemplate.opsForValue().set("neosis:bulk:discovered", String.valueOf(filesDiscovered.get()));
            redisTemplate.opsForValue().set("neosis:bulk:processed", String.valueOf(filesProcessed.get()));
            redisTemplate.opsForValue().set("neosis:bulk:failed", String.valueOf(filesFailed.get()));
            redisTemplate.opsForValue().set("neosis:bulk:assertions", String.valueOf(assertionsGenerated.get()));
            redisTemplate.opsForValue().set("neosis:bulk:edges", String.valueOf(edgesGenerated.get()));
            redisTemplate.opsForValue().set("neosis:bulk:backlog", String.valueOf(queueBacklog.get()));
            redisTemplate.opsForValue().set("neosis:bulk:throughput", String.valueOf(throughputDocsSec));
            redisTemplate.opsForValue().set("neosis:bulk:eta", String.valueOf(etaSeconds));
        } catch (Exception e) {
    ...
    public void incrementDiscovered() {
        ...
        redisTemplate.opsForValue().set("neosis:bulk:discovered", String.valueOf(val));
        recalc();
    }

    public void incrementProcessed() {
        int val = filesProcessed.incrementAndGet();
        redisTemplate.opsForValue().set("neosis:bulk:processed", String.valueOf(val));
        recalc();
    }

    public void incrementFailed() {
        int val = filesFailed.incrementAndGet();
        redisTemplate.opsForValue().set("neosis:bulk:failed", String.valueOf(val));
        recalc();
    }

    public void addAssertions(int count) {
        int val = assertionsGenerated.addAndGet(count);
        redisTemplate.opsForValue().set("neosis:bulk:assertions", String.valueOf(val));
        broadcast();
    }

    public void addEdges(int count) {
        int val = edgesGenerated.addAndGet(count);
        redisTemplate.opsForValue().set("neosis:bulk:edges", String.valueOf(val));
        broadcast();
    }

    public void setQueueBacklog(int backlog) {
        queueBacklog.set(backlog);
        redisTemplate.opsForValue().set("neosis:bulk:backlog", String.valueOf(backlog));
    }
